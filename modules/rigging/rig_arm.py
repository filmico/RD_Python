...
armIKCtrl = {}
armIKCtrl['grpOffsetDetails'] = ['ik_arm_grp']  # Name of the grp
armIKCtrl['ctrlShapeDetails'] = ['ik_arm_ctl', [1, 0, 0], [0, 0, 0]]  # name, normal, center
...
def createControl(controlInfo):

    # Create Control GRP
    cmds.group(empty=True, name=controlInfo['grpOffsetDetails'][0])
    # Create Shape to be placed under the offset group
    cmds.circle(name=controlInfo['ctrlShapeDetails'][0],
                normal=controlInfo['ctrlShapeDetails'][1],
                center=controlInfo['ctrlShapeDetails'][2])
    # Delete Construction History
    cmds.delete(constructionHistory=True)
    # Parent the control under the group
    # cmds.parent(controlInfo['ctrlShapeDetails'][0], controlInfo['grpOffsetDetails'][0])
    cmds.select(clear=True)
...
createControl(armIKCtrl)

# Parent the control under the group
cmds.parent(armIKCtrl['ctrlShapeDetails'][0], armIKCtrl['grpOffsetDetails'][0])
cmds.select(clear=True)

# Move armIK Control to Wrist Position
posConstraint([armIKCtrl['ctrlShapeDetails'][0], armIK[2][0]], world=True)
